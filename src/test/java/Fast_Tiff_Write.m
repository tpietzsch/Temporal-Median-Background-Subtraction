classdef Fast_Tiff_Write  < handle
    ...
    methods
        function obj = Fast_Tiff_Write(filename,pixelsize)
        ...
        function WriteIMG(obj,img)
            ...
            if isempty(obj.TagList) %construct the TagList from this img
                ...
                obj.TagList(37:39) = obj.TifTag(obj,'ResolutionUnit','short',1,3);%pixels per cm
    ...
    methods (Static, Access=private)
        function [out] = zip_bytes(input,level)
            output      = java.io.ByteArrayOutputStream();
            compresser  = java.util.zip.Deflater(level);
            outstrm     = java.util.zip.DeflaterOutputStream(output,compresser);
            outstrm.write(typecast(input,'uint8'));
            compresser.finish();
            outstrm.close();
