    ...
    public static void main(String[] args) {
        ...
        for (int ithread = 0; ithread < threads.length; ithread++) {
            threads[ithread] = new Thread() { //make threads
                ...
                public void run() {
                    final RandomAccess<UnsignedShortType> front = ranked.randomAccess();
                    final RandomAccess<UnsignedShortType> back = img.randomAccess();
                    final MedianHistogram[] medians = new MedianHistogram[imgw];
                    Arrays.setAll(medians, i -> new MedianHistogram(window, rankmap.getMaxRank()));
                    for (int y = ai.getAndIncrement(); y < imgh; y = ai.getAndIncrement()) { //get unique i
                        final int[] pos = { 0, y, 0 };
                        ...
                        for (int i = 0; i < window; ++i) {
                            for ( int x = 0; x < imgw; ++x) {
                                medians[x].add((short) front.get().get());
                                front.fwd(0);
                            }
                            front.move(-imgw, 0);
                            front.fwd(2);
                        ...
                        for (int i = 0; i <= windowC; ++i) {
                            for ( int x = 0; x < imgw; ++x) {
                                ...
                                t.set(t.get() + offset - rankmap.fromRanked(medians[x].get()));
                                back.fwd(0);
                            }
                            back.move(-imgw, 0);
                            back.fwd(2);
